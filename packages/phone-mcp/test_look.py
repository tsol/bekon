...
class LookTests(unittest.TestCase):
    ...
    def test_format_look_hides_chrome_by_default(self):
        ...
        text = format_look(snap, HOME_ITEMS, "/tmp/phone-mcp/x.jpg", 300_000, include_chrome=False)
        self.assertIn("Instagram", text)
        self.assertNotIn("12:41", text)
        self.assertNotIn("Главный экран", text)
        self.assertIn("home: true", text)
        self.assertIn("screenshot: /tmp/phone-mcp/x.jpg", text)
    ...
    def test_hermes_visible_path_rewrites_workspace(self):
        host = Path(__file__).resolve().parents[4]
        sample = str(host / "projects" / "wlya-tunnel" / "phone-manager" / "mcp" / "shots" / "a.jpg")
        out = hermes_visible_path(sample)
        self.assertEqual(
            out,
            "/opt/data/workspace/projects/bekon/packages/phone-mcp/shots/a.jpg",
        )

    def test_hermes_visible_path_leaves_tmp(self):
        self.assertEqual(hermes_visible_path("/tmp/phone-mcp/x.jpg"), "/tmp/phone-mcp/x.jpg")
